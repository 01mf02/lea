...
public class ConstantGenerator {
	...
	public void generate(CodeWriter cw) throws IOException {
		for (Map.Entry<String, Constant> entry : cstTable.entrySet()) {
			...
			if (entry.getValue().getType().toString()
					.equals(new EnumType().getType().toString()))
				cw.writeLine("//public final "
						+ entry.getValue().getType().toJava() + " "
						+ ng.generateName(entry.getKey()) + " = "
						+ entry.getValue().toJava() + ";");
			else if (entry.getValue().getType().toString()
					.equals(new TupleType().getType().toString()))
